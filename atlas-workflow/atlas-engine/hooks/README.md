# Atlas v2 — Optional hooks layer (mechanical stall enforcement)
...
`.claude/workflows/atlas.mjs`) is:

```
atlas/initiatives/<initiativeId>/.stall-signal.json
```

The hook discovers this path via the `ATLAS_STALL_SIGNAL` env var (the engine exports it; §3).
If the var is unset the hook is a no-op — so the same global hook stays silent outside a Atlas run.

**Shape** (the contract — keep it stable; the engine reads exactly these fields):

```jsonc
{
  "sliceId": "p1-s3",            // optional; the active slice if ATLAS_SLICE_ID is set, else null
...
| Field     | Type             | Meaning |
| --------- | ---------------- | ------- |
| `sliceId` | `string \| null` | Active slice id (`ATLAS_SLICE_ID`) or `null`. |
...
approach (atlas.mjs `HANDS_OFF`). **The engine deletes the file once consumed**; the hook only
    ...
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "node /ABSOLUTE/PATH/TO/atlas-engine/hooks/stall-hook.mjs"
          }
        ]
      }
    ]
...
| ---------------------- | ------ | ------- |
| `ATLAS_STALL_SIGNAL`  | engine | Absolute path to write the signal file. **Unset ⇒ hook is a no-op.** |
| `ATLAS_FOOTPRINT`     | engine | Absolute path to a newline-delimited list of allowed path globs for the active slice. Absent ⇒ footprint-escape detection is skipped (repeat detection still runs). |
| `ATLAS_SLICE_ID`      | engine | Active slice id, copied into `sliceId`. Optional. |
| `ATLAS_STALL_WINDOW`  | you    | Sliding-window size. Default `12`. |
| `ATLAS_STALL_REPEATS` | you    | Repeat threshold. Default `3`. |
...
`atlas-engine/hooks/stall-hook.mjs` and point the snippet at it.

```js
#!/usr/bin/env node
// atlas-engine/hooks/stall-hook.mjs
// Atlas v2 OPTIONAL PostToolUse stall sensor. Non-blocking by construction: always exits 0,
// never returns a block decision. No-op unless ATLAS_STALL_SIGNAL is set (so it is safe global).
import { readFileSync, writeFileSync, existsSync } from 'node:fs'
import { createHash } from 'node:crypto'

const out = process.env.ATLAS_STALL_SIGNAL
if (!out) process.exit(0)                                 // not in a Atlas run → silent

const WINDOW  = Number(process.env.ATLAS_STALL_WINDOW  || 12)
const REPEATS = Number(process.env.ATLAS_STALL_REPEATS || 3)
...
- **What it costs:** the hook is **GLOBAL to the Claude Code session**, not scoped to a Atlas run.
  Once in `settings.json` it fires on *every* `Edit`/`Write`/`Bash` in that session. It is designed
  to be inert outside a run (no-op unless `ATLAS_STALL_SIGNAL` is set, always exits 0,
  ...
  **Enable deliberately; prefer project-level `.claude/settings.json` over user-level** so it lives
  and dies with the repo where you run Atlas.
