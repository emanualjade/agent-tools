export const meta = {
  name: 'review-atlas-hardening',
  description: 'Adversarial pass over the hardened Atlas v2: regression check on the engine S0 preflight floor + hand-applied edits, dead-vestige/schema-consumption check, cross-file coherence, each-of-9-failures-closed, lean/new-conflicts, autonomous-safety — verify findings, synthesize a ready-for-dogfood verdict.',
  phases: [
...
const base = 'You are adversarially reviewing the just-HARDENED Atlas v2 build workflow for regressions and remaining holes. Read what your lens needs:\n  - engine: ' + ENGINE + '\n  - contract: ' + DESIGN + '\n  - modules under ' + ROOT + ' (disciplines/, surfaces/, steps/, front-door/)\n\n' + RULE + '\n\nThe hardening added (across 11 files + engine + DESIGN): `no_substitution` (honest-verification, the spine), `environment-scoped` (S5), `behavior-test-committed`/`-present` (S4/S5), `no-silent-workaround`/`capability-unavailable` (S4), `sibling-surveyed` (read-before-write) + `follow-the-house-pattern` (S2), web-backend `Through-the-tool` migrations + obstruction-loop 5th trigger, grill `## Required Capabilities & Preflight`/`## Environments` + launch-blocker, `atlas.md` floored skip, `disciplines_attested` (S6/S7), an engine S0 PREFLIGHT FLOOR (S0 returns verdict:BLOCKED if idea-decisions.md has unruled surfaces / un-provisioned capabilities; runInitiative degrades on s0.verdict===BLOCKED), and DESIGN non-negotiable #12.\n\nYOUR LENS: '
