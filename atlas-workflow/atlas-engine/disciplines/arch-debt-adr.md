...
- **`-> <adr-link>`** — `->` then the ADR's repo-relative path:
  `atlas/initiatives/<id>/adr/NNN-slug.md`. The pointer is mandatory; a markerless-of-ADR or
...
```
// ARCH-DEBT(P2-S3): sync write blocks on email send; needs an async outbox seam -> atlas/initiatives/checkout/adr/004-email-outbox-seam.md
```
...
One file per decision: `atlas/initiatives/<id>/adr/NNN-slug.md`. `NNN` is a zero-padded
...
- **`adr-committed`** — a file exists at `atlas/initiatives/<id>/adr/NNN-slug.md`, is in the commit
