---
name: grill-idea
description: Pressure-test a refined idea until the consequential decisions are locked — especially the ones the autonomous build would otherwise guess. Produces the decision log + strike-v2 build launch args. Step 2 of the front door.
allowed-tools: Read Write Edit Grep Glob WebFetch WebSearch
---

# Grill Idea — strike-v2 front door (step 2 of 2)

Pressure-test the refined idea until the consequential decisions are explicit — *interactively, one
question at a time.*

**The whole point:** the strike-v2 build runs **hands-off** and **cannot ask you mid-run** — where it
...
Write the decision log to `strike/initiatives/<id>/idea-decisions.md`:
...
```md
## Build Launch Args (for strike-v2)
initiativeId:   <kebab-id>   ← LOAD-BEARING. This is the field that binds the front door to the build. It
                               ...
                               the build lands in strike/initiatives/<id>/ and reads those files by id.
                               If omitted, the build runs as "initiative" (the default), writes to a
                               DIFFERENT strike/initiatives/ dir, and orphans this decision log — the build
...
`.claude/workflows/strike-v2.mjs` in this project, and `args` = the object above.
...
`strike/initiatives/` dir, and **orphans this decision log — the build then cannot find your rulings by
