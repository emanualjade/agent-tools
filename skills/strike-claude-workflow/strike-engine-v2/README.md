# Strike v2 — the optimized feature-building workflow

The build workflow we use for production features. A research-grounded successor to the v1 Strike
...
- **Engine:** `.claude/workflows/strike-v2.mjs`
...
- **v1 (reference):** `.claude/workflows/strike-build-engine.mjs` + `strike-engine/`
...
1. **`/strike <vague idea>`** — the `refine-idea` then `grill-idea` skills brainstorm + pressure-test
...
2. **It launches the build** — `Workflow` with `scriptPath: .claude/workflows/strike-v2.mjs` and the
...
```jsonc
{
  ...
  "rootDir":               "/Users/cracklehat/Sites/workflow-exploration/strike-engine-v2",
  "maxFixAttempts":        3,
  "maxRouteBacks":         4,
  "maxSplitsPerPhase":     6,
...
- **Build output** — `main-spec.md`, `acceptance-criteria.md`, phase/slice artifacts, **ADRs under
  `strike/initiatives/<id>/adr/`**, and the **actual code changes** — land in the **cwd the workflow
  is launched from** (your target repo). Launch from the project you're building in; running from this
  sandbox just scaffolds a `strike/` tree here.
...
.claude/workflows/strike-v2.mjs   # the engine
```
