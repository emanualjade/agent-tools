...
const fixVerdicts = (await parallel(idx.map((i) => () =>
  agent('You are confirming that a specific fix from round 1 of an adversarial review actually landed in the patched Strike v2 engine.\n\n' + SCHEMA_RULE +
...
phase('Fresh sweep')
const base = 'Fresh adversarial sweep of the PATCHED Strike v2 engine (' + ENGINE + '), contract ' + DESIGN + ', and modules under ' + ROOT + '. ' + SCHEMA_RULE + '\n\nFocus your lens; report only REAL defects (a clean lens returns empty). YOUR LENS: '
const LENSES = [
...
const synthesis = await agent('Synthesize the round-2 re-review of Strike v2 into a production-readiness verdict.\n\nDATA (json: per-fix verdicts from round-1 fixes, plus newly-confirmed fresh findings):\n' + payload +
