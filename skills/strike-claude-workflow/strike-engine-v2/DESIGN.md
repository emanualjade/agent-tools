...
Derived from `strike-engine/research/` (8-dimension failure-mode research + 3-design judge panel).
v1 (the faithful port at `.claude/workflows/strike-build-engine.mjs` + `strike-engine/`) is
...
strike-engine-v2/
  ...
  front-door/               # INTERACTIVE layer — runs in the conversational agent, NOT the workflow
    ...
    strike.md               # the /strike command: refine -> grill -> launch the build, one flow
...
.claude/workflows/strike-v2.mjs   # THE engine (authored against this contract)
```

Runtime artifacts (written into the target repo, cwd-relative) keep v1's durable layout under
`strike/initiatives/<id>/...`, **plus** `adr/` and an `ARCH-DEBT` convention (section 7).
...
`strike/initiatives/<id>/adr/NNN-*.md`. The next slice's read-before-write greps `ARCH-DEBT`
...
## 10. Engine responsibilities (`.claude/workflows/strike-v2.mjs`)
...
- Skills/modules read from an absolute `enginirootDir` (default the absolute path of
  `strike-engine-v2/`); artifacts + code written into cwd (the target repo). Hands-off escalation
...
- **Front door (interactive — `front-door/`):** `refine-idea` and `grill-idea` are skills the
  *conversational agent* runs, because only that layer can ask the user questions. They turn a vague
  ...
  engine treats conservatively. The `/strike` command runs refine → grill → launch as one flow.
...
This is the full original-Strike shape (refine → grill → build → verify) mapped onto the right layers.

## Migration from v1

v1 stays in place (`.claude/workflows/strike-build-engine.mjs`, `strike-engine/`) as reference. When v2
