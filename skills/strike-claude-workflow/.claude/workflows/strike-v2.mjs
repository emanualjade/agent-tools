export const meta = {
  name: 'strike-v2',
  description: 'Strike v2 build engine: risk-routed FAST/FULL lanes, self-correcting architectural-obstruction loop with real upstream re-entry, surface-triggered guardrails, honest verification. Refine+grill happen with you first; this runs everything after.',
...
/*
 * STRIKE v2 — optimized build engine. Conforms to strike-engine-v2/DESIGN.md.
 *
...
const a = args || {}
const cfg = {
  rootDir: a.rootDir || '/Users/cracklehat/Sites/workflow-exploration/strike-engine-v2',
...
async function runStep({ step, label, phaseGroup, schema, task }) {
  ...
  report.agentCalls++
  const prompt = `You are executing Strike v2 step "${step}" for initiative "${initiative.id}" (${initiative.name}).
