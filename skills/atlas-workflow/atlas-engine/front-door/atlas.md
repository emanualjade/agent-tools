...
3. **Build** — call the **Workflow** tool with `scriptPath` = `.claude/workflows/atlas.mjs` in this
   project and `args` = the launch args from step 2. Set `args.initiativeId` to the SAME `<id>` the
   refine/grill step used for `atlas/initiatives/<id>/` (`refined-idea.md` + `idea-decisions.md`) — do NOT
